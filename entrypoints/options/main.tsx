  ...
  return <main>
    ...
    <section><div className="section-title"><span>01</span><h2>Dein Entscheidungsmodell</h2></div>
      <form onSubmit={e => { e.preventDefault(); void action(() => rpc('settings', { settings }), 'Einstellungen gespeichert.'); }}>
        ...
        <div className="privacy"><h3>Was die KI erhält</h3><p>Bereinigte Ressourcen-URLs, Elementmerkmale, kurze Werbehinweise und technische Beobachtungen zu Player-Popups und Overlays. Keine Cookies, Formulareingaben, Chatverläufe oder vollständigen Seiten. Die besuchten Domains werden dabei an TypeSafe übermittelt.</p><p>Player-Popups werden lokal beim Öffnungsversuch abgefangen. Jev lernt dazu passende Quellen und Overlays; unbekannte oder unsichere Werbung kann zunächst sichtbar bleiben.</p></div>
    ...
    <section><div className="section-title"><span>02</span><h2>Gelernte Werbequellen</h2></div><p>Regeln gelten für die jeweilige Website und ihre Subdomains. Freigaben verhindern erneutes KI-Blockieren; Filterlisten werden dadurch nicht überschrieben.</p>
      ...
      {!rules.length ? <div className="empty">{state.rules.length ? 'Keine passenden Regeln.' : 'Noch nichts gelernt. Nach dem Aktivieren erkennt Jev neue Werbequellen beim Surfen.'}</div> : <ul className="rules">{rules.map(rule => <li key={rule.id}><div><strong>{rule.site}</strong><code>{rule.url}{rule.selector ? ` · ${rule.selector}` : ''}</code><small>{rule.type} · Werbung {Math.round(rule.ad * 100)} % · {rule.model} · {rule.origin === 'behavior' ? 'Player-Verhalten' : rule.origin === 'dom' ? 'Element + URL' : 'Netzwerk-Metadaten'}</small></div><button className="secondary" disabled={busy} onClick={() => void action(() => rpc('allow', { id: rule.id }), 'Quelle freigegeben. Seite neu laden, um zuvor blockierte Inhalte abzurufen.')}>Freigeben</button></li>)}</ul>}
    ...
    </section><footer>AI Adblocker · GPL-3.0 · Lokal gelernt, lokal gespeichert.<br />Keine Telemetrie. Kein Schutz in privaten Fenstern. YouTube-Schutz für getrennte Werbeeinblendungen. Fest in den Videostream integrierte Werbung bleibt ausgenommen.</footer>
  </main>;
}
